...
// der Aenderungsliste, und die Versionspille ist weg. NACHWUCHS_CHANGELOG
...
function zeigeInfo() {
  ...
  if (funktionen) {
    funktionen.innerHTML = NACHWUCHS_FUNKTIONEN.map((g) =>
  ...
  if (!verlauf) return;
  verlauf.innerHTML = NACHWUCHS_CHANGELOG.map((block) =>
    block.groups.map((g) =>
