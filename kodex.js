...
// der Aenderungsliste, und die Versionspille ist weg. KODEX_CHANGELOG
...
function zeigeInfo() {
  ...
  if (funktionen) {
    funktionen.innerHTML = KODEX_FUNKTIONEN.map((g) =>
  ...
  if (!verlauf) return;
  verlauf.innerHTML = KODEX_CHANGELOG.map((block) =>
    block.groups.map((g) =>
